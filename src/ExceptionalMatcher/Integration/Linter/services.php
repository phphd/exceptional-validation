...
return static function (ContainerConfigurator $configurator, ContainerBuilder $container): void {
    ...
    $services
        ...
        ->args([
            service('phd_exceptional_matcher.linter.property_plan_compiler'),
    ...
    $services
        ->set('phd_exceptional_matcher.linter.property_plan_compiler', PropertyExceptionMappingPlanCompiler::class)
        ->args([
            service('phd_exceptional_matcher.linter.catch_plan_compiler'),
            // nested plans go through the lint-mode registry too, not the production one
            service('phd_exceptional_matcher.linter.plan_registry'),
            false, // $throwOnFailure
            service(MappingDefectCollector::class),
        ])
        // it asks for the registry that is built from it: being lazy is what lets the registry be built first
        ->lazy($lazy(ExceptionMappingPlanCompiler::class))
    ;

    $services
        ->set('phd_exceptional_matcher.linter.catch_plan_compiler', CatchExceptionMappingPlanCompiler::class)
        ->args([
